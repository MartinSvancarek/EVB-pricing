...
```bash
npm install
npm run db:push     # vytvoří SQLite DB ze schema.prisma
npm run db:seed     # naplní vzorovými daty (60 dní usage, FX, revenue, scénář)
npm run dev         # http://localhost:3000
```

Pro reset dat: `npm run db:reset`.

Tech stack: Next.js 15 (App Router), TypeScript, Prisma + SQLite (lze přepnout na Postgres změnou `provider` v [prisma/schema.prisma](prisma/schema.prisma)), Tailwind, Recharts.
